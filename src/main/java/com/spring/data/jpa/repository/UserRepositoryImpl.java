...
public class UserRepositoryImpl {
	...
	// Custom Method in SpringDataJPA
	public static void getUsersBySalary(int salary, UserRepository repository) {
	...
	public static void update(int id, UserRepository repository, String name, char gender, int salary) {
		if (repository.existsById(id)) {
			Optional<User> optional = repository.findById(id);
			User user = optional.get();
			user.setName(name);
	...
	public static void saveSingelObject(UserRepository repository, String name, char gender, int salary) {
		User user = new User();
		user.setName(name);
		user.setGender(gender);
		user.setSalary(salary);
